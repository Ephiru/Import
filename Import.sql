...
SELECT * FROM AttachedProduct;
SELECT * FROM Client;
SELECT * FROM ClientService;
SELECT * FROM DocumentByService;
SELECT * FROM Gender;
...
SELECT * FROM client_b_import;
SELECT * FROM serviceclient_b_import;
SELECT * FROM service_b_import;
SELECT * FROM Gender_ids;

INSERT INTO Gender (Name)
SELECT DISTINCT Пол FROM client_b_import;

CREATE TABLE Gender_ids
(
    code int IDENTITY(1, 1),
...
INSERT INTO Gender_ids (code_id)
SELECT DISTINCT Пол FROM client_b_import;

ALTER TABLE Gender
ADD FOREIGN KEY(Name) REFERENCES Gender_ids(code_id);

ALTER TABLE client_b_import
ALTER COLUMN Пол nvarchar(10);

ALTER TABLE client_b_import
ADD FOREIGN KEY(Пол) REFERENCES Gender_ids(code_id);

INSERT INTO Client (FirstName, LastName, Patronymic, Birthday, RegistrationDate, Email, Phone, GenderCode)
SELECT Имя, Фамилия, Отчество, Дата_рождения, Дата_регистрации, Email, Телефон, Gender.Code FROM client_b_import, Gender, Gender_ids 
WHERE Gender.Name = Gender_ids.code_id and Gender_ids.code_id = client_b_import.Пол;

ALTER TABLE Service
ALTER COLUMN Cost nvarchar(50);

ALTER TABLE Service
ALTER COLUMN Title nvarchar(50);

ALTER TABLE Service
ALTER COLUMN DurationInSeconds nvarchar(50);

ALTER TABLE Service
ALTER COLUMN Discount nvarchar(50);

INSERT INTO Service(Title, Cost, DurationInSeconds, Discount)
SELECT Наименование_услуги, Стоимость, Длительность, Действующая_скидка FROM service_b_import

ALTER TABLE ServicePhoto
ALTER COLUMN PhotoPath nvarchar(100);
